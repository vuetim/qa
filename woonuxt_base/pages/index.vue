<script lang="ts" setup>
const { data, error } = await useAsyncGql('getProductCategories', { first: 6 });

const productCategories = data.value.productCategories?.nodes as ProductCategory[];

useHead({
  title: `Home | Qafore`,
  meta: [
    {
      name: 'description',
      content:
        'Discover timeless elegance and curated style at qafore.com. Explore our exquisite collection of necklaces and accessories that seamlessly blend sophistication with trendsetting designs',
    },
  ],
  link: [{ rel: 'canonical', href: 'https://qafore.com/' }],
});
</script>

<template>
  <main>
    <HeroBanner />

    <section class="container my-16">
      <div class="flex items-end justify-between">
        <h2 class="text-lg font-semibold md:text-2xl">{{ $t('messages.shop.shopByCategory') }}</h2>
        <NuxtLink class="text-primary" to="/categories">{{ $t('messages.general.viewAll') }}</NuxtLink>
      </div>
      <div class="grid justify-center grid-cols-2 gap-4 mt-8 md:grid-cols-3 lg:grid-cols-6">
        <CategoryCard v-for="(category, i) in productCategories" :key="i" class="w-full" :node="category" />
      </div>
      <ProductSection />
    </section>
    <!-- Features -->
    <section class="container xl:grid xxl:grid 2xl:grid gap-4 my-20 md:grid-cols-2 lg:grid-cols-4">
      <div class="flex items-center gap-8 p-8 rounded-lg bg-gray-100">
        <img src="../public/icons/box.svg" width="60" height="60" alt="Free Shipping" loading="lazy" />
        <div>
          <h3 class="text-xl font-semibold">Free Shipping</h3>
          <p class="text-sm">Free shipping on order over €50</p>
        </div>
      </div>
      <div class="flex items-center gap-8 p-8 bg-gray-100 rounded-lg">
        <img src="../public/icons/moneyback.svg" width="60" height="60" alt="Money Back" loading="lazy" />
        <div>
          <h3 class="text-xl font-semibold">Peace of Mind</h3>
          <p class="text-sm">30 days money back guarantee</p>
        </div>
      </div>
      <div class="flex items-center gap-8 p-8 bg-gray-100 rounded-lg">
        <img src="../public/icons/secure.svg" width="60" height="60" alt="Secure Payment" loading="lazy" />
        <div>
          <h3 class="text-xl font-semibold">100% Secure</h3>
          <p class="text-sm">Your payment are safe with us.</p>
        </div>
      </div>
      <div class="flex items-center gap-8 p-8 bg-gray-100 rounded-lg">
        <img src="../public/icons/support.svg" width="60" height="60" alt="Support 24/7" loading="lazy" />
        <div>
          <h3 class="text-xl font-semibold">Support 24/7</h3>
          <p class="text-sm">24/7 Online support</p>
        </div>
      </div>
    </section>
    <!-- <div class="container flex flex-wrap items-center justify-center my-16 text-center gap-x-8 gap-y-4 brand lg:justify-between">
      <NuxtImg src="/images/logoipsum-211.svg" alt="Brand 1" width="132" height="35" />
      <NuxtImg src="/images/logoipsum-221.svg" alt="Brand 2" width="119" height="30" />
      <NuxtImg src="/images/logoipsum-225.svg" alt="Brand 3" width="49" height="48" />
      <NuxtImg src="/images/logoipsum-280.svg" alt="Brand 4" width="78" height="30" />
      <NuxtImg src="/images/logoipsum-284.svg" alt="Brand 5" width="70" height="44" />
      <NuxtImg src="/images/logoipsum-215.svg" alt="Brand 6" width="132" height="40" />
    </div> -->
  </main>
</template>

<style scoped>
.brand img {
  max-height: min(8vw, 120px);
  object-fit: contain;
  object-position: center;
}
</style>
